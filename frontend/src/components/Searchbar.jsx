    ...
    return (
        <div ref={wrapperRef} className="relative w-full max-w-xl">
            <div className={`flex items-center gap-2.5 rounded-xl border px-3.5 py-2.5 transition-all duration-200 ${
                focused
                    ? 'border-[#2CFF05]/30 bg-[#0d1512]'
                    : 'border-white/[0.06] bg-[#0a100e]'
            }`}>
                {loading ? (
                    <div className="h-4 w-4 flex-shrink-0 animate-spin rounded-full border-2 border-zinc-700 border-t-[#2CFF05]" />
                ) : (
                    <Search size={15} className={`flex-shrink-0 transition-colors duration-200 ${focused ? 'text-[#2CFF05]' : 'text-zinc-600'}`} />
                )}
                <input
                    value={query}
                ...
                {query && (
                    <button
                        ...
                        className={`relative flex-shrink-0 transition-colors ${
                            filterOpen || activeFilterCount > 0 ? 'text-[#2CFF05]' : 'text-zinc-600 hover:text-zinc-400'
                        }`}
                    >
                        <SlidersHorizontal size={14} />
                        {activeFilterCount > 0 && (
                            <span className="absolute -top-1.5 -right-1.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-[#2CFF05] text-[9px] font-medium text-black">
                                {activeFilterCount}
                            </span>
                        )}
                    </button>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select
                                ...
                                className="w-full rounded-lg border border-white/[0.06] bg-[#0a100e] px-2.5 py-1.5 text-[12px] text-zinc-300 focus:border-[#2CFF05]/30 focus:outline-none"
                            >
